...
[模型说明](MODEL_CARD.md) · [安全政策](SECURITY.md) ·
...
- 支持导入与识别进度、逐文件处理状态、自动跟随当前处理项和跨重启持久日志。
- 加入小人物、背影和多人场景复检，并通过二次确认降低纯产品、宠物和手脚局部的误检。
...
仓库中的 [Windows 构建工作流](.github/workflows/build-windows.yml) 会在
GitHub 托管的 Windows 环境中安装锁定版本依赖、下载公开的 ExifTool、运行测试、
构建 EXE，并为构建产物生成 SHA-256 校验文件。工作流可手动运行，也会在推送
`v*` 版本标签时运行。
...
项目当前发布物尚未获得可信代码签名。SignPath Foundation 开源项目申请已于
2026-07-30 提交，目前等待人工审核。请仅从本仓库的 Releases 页面获取官方
发布物，并核对随附的 SHA-256。
